...
public class SubwayJoinMapper extends Mapper<LongWritable, Text, IntWritable, Text> {
        ...
        loop: for (Map.Entry<String, double[]> entry : subwayLocations.entrySet()) {
            ...
            if (latitude <= entry.getValue()[0] + LAT_LONG_RANGE && latitude >= entry.getValue()[0] - LAT_LONG_RANGE) {
                ...
                if (longitude <= entry.getValue()[1] + LAT_LONG_RANGE
                    ...
                    final String outString = temp[2] + ",1";
                    context.write(new IntWritable(Integer.parseInt(entry.getKey())), new Text(outString));
                    break loop;
                }
            }
        }
