...
public class ComplaintJoinMapper extends Mapper<LongWritable, Text, NullWritable, Text> {
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        final double latitude = Double.parseDouble(temp[7].substring(2));
        final double longitude = Double.parseDouble(temp[8].substring(0, temp[8].length() - 2));
        loop: for (Map.Entry<String, double[]> entry : subwayLocations.entrySet()) {
            ...
            if (latitude <= entry.getValue()[0] + LAT_LONG_RANGE && latitude >= entry.getValue()[0] - LAT_LONG_RANGE) {
                ...
                if (longitude <= entry.getValue()[1] + LAT_LONG_RANGE
                    ...
                    final String outVal = temp[0] + "," + temp[1] + "," + temp[2] + "," + temp[3] + "," + temp[4] + ","
                            + temp[5] + "," + temp[6] + "," + latitude + "," + longitude;
                    context.write(NullWritable.get(), new Text(outVal + "," + entry.getKey()));
                    break loop;
                }
            }
        }
